...
export default class EnergyMeter extends Component {
  ...
  onFinish = (values) => {
    ...
    let pay = {
      ...
      // "redirectUrl": "service",
      // "serviceName": "ENERGY",
                "energyMeter":values.energyMeter.energyMeter,
                "energyConsumption": this.state.energyMeterData[0].energyConsumption,
    ...
    axios.post("http://localhost:8080/api/billing/save", pay, {headers}).then(res=>{
      if (res.data) {
        // this.setState({object: res.object});
        this.setState({ energyBillingData: res.data})
        //console.log("saved billing data", res.data)
        let billingDatas =  {
          "meterId": this.state.energyBillingData.id,
          "energyConsumption": this.state.energyBillingData.energyConsumption,
          "amountEnergyConsumption": this.state.energyBillingData.amountEnergyConsumption,
          "sum": this.state.energyBillingData.sum,
    ...
    return (
      ...
      <React.Fragment>
       ...
       <Table
          ...
          dataSource={this.state.energyBillingDataResult}
          bordered
